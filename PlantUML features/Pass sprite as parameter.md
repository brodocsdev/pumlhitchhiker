...
![](/PlantUML%20features/PassSpriteAsParameter/option1.png)
...
This option uses the unquoted keyword. The caller of the procedure does
not need to add quotes when calling the procedure.



![](/PlantUML%20features/PassSpriteAsParameter/option2.png)
...
![](/PlantUML%20features/PassSpriteAsParameter/option3.png)
...
We said previously that the current stdlib macros are largely the same
across files - changing for each icon.

So now we have a way to define a procedure(s) in one place, and pass an
icon.
