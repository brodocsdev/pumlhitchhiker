...
- [PlantUMLPreProcessor](https://plantuml.com/preprocessing) like
  ...
  > The example diagram here shows all of these annotations in one
  > diagram.
  >
  > In general, "less is more", so use these annotations if/where they
  > add value and clarity to the audience.

## Notes

Notes can be added to diagrams. These can include
<https://plantuml.com/creole> syntax e.g. for bullets, text formatting.

Creole Sprites can be added to Notes. But this causes bullets to stop
working. So, it's creole sprites or bullets but not both.
...
Handwritten gives a wavy effect to diagrams. This can be useful to
differentiate between e.g. a raw proposal versus an approved proposal.
It can be set to true or false ala *skinparam handwritten true*.

## Title

Various formatting options are available per
